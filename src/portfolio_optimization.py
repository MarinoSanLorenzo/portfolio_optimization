...
def get_best_and_worst_scenarios(
    scenarios_portfolio: np.array,
    lower_quantile_lvl: float = 0.05,
    upper_quantile_lvl: float = 0.95,
) -> Scenarios:
...
def get_df_simulated_stock(
    stock_name: str, simulated_stocks: dict, params: dict
) -> pd.DataFrame:
...
def add_sharpe_ratio(portfolios_simulated: pd.DataFrame) -> pd.DataFrame:
    rf = 0.01  # risk factor
    portfolios_simulated["sharpe_ratio"] = (
        portfolios_simulated["returns"] - rf
    ) / portfolios_simulated["volatility"]
    return portfolios_simulated


def get_portfolio_with(
    ...
    if lowest_volatility and not (highest_return):
        portfolio = portfolios_simulated.iloc[
            portfolios_simulated["volatility"].idxmin()
        ]
        ...
        portfolio = portfolios_simulated.iloc[
            portfolios_simulated["sharpe_ratio"].idxmax()
        ]
    else:
...
def get_investment_summary(portfolios_simulated: pd.DataFrame) -> list:
    ...
    portfolio_with_highest_return = get_portfolio_with(
        portfolios_simulated,
    ...
    summary_msg3 = html.Ul(
        children=[
            html.Li(f"{k}:\t{v}") for k, v in portfolio_with_lowest_volatility.items()
        ]
    )
    summary_msg4 = "The portfolio with highest returns is presented as follow:\n"
    summary_msg5 = html.Ul(
        children=[
            html.Li(f"{k}:\t{v}") for k, v in portfolio_with_highest_return.items()
        ]
    )
